...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    # TI BAI: 09/23/2022
    config_file_ = list(config_file_)
    config_file_ = [x.replace('\n', '\\n').replace('\t', '\\t') for x in config_file_]
    ...
    train_loader = DataLoader(train_ds, num_workers=12, batch_size=num_images_per_batch, shuffle=True)
    val_loader = DataLoader(val_ds, num_workers=12, batch_size=1, shuffle=False)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_data in train_loader:
            ...
            end_time = time.time()
            if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                print(f"[{str(datetime.now())[:19]}] " + f"Data time: {data_time:.2f}, Batch time: {batch_time:.2f} " + f"{step}/{epoch_len}, train_loss: {loss.item():.4f}")
