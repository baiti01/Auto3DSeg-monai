...
class SegresnetAlgo(BundleAlgo):
    def fill_template_config(self, data_stats_file, output_path, **kwargs):
        ...
        for yaml_file, yaml_contents in fill_records.items():
            ...
            #Ti Bai, expand the list otherwise an error will be raised when training
            if 'transforms' in yaml_file:
                k1, k2 = k.split('#')[:2]
                new_transforms = []
                old_transforms = parser.config[k1][k2]
                for i in old_transforms:
                    if isinstance(i, list):
                        for j in i:
                            new_transforms.append(j)
                    else:
                        new_transforms.append(i)
                parser.config[k1][k2] = new_transforms
            # ##########
